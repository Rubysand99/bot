...
class SettingsView(View):
    ...
    async def builder(self,  i, b): await self._send_role_select(i, "cfg_builder_role",   "Builder Role")
    @discord.ui.button(label="🎖️ Done Role",       style=discord.ButtonStyle.secondary, row=1)
    async def done_role(self, i, b): await self._send_role_select(i, "cfg_done_role",     "Done Role")
    ...
    @discord.ui.button(label="📄 Transcript Channel", style=discord.ButtonStyle.secondary, row=2)
    async def transcript(self, i, b): await self._send_channel_select(i, "cfg_transcript_channel", "Transcript Channel", "Kênh lưu transcript khi đóng ticket")
