...
class LoggerCog(commands.Cog):
    ...
    async def _send_daily_report(self):
        ...
        # ── Balance: giao dịch trong kênh balance 24h qua ──
        bal = get_balance_data()
        bal_history = bal.get("history", [])
        bal_in_24h = bal_out_24h = bal_fee_24h = bal_tx_24h = 0
        for tx in bal_history:
            try:
                # history lưu dạng "dd/mm/yyyy HH:MM UTC"
                tx_dt = datetime.strptime(tx["time"], "%d/%m/%Y %H:%M UTC").replace(tzinfo=timezone.utc)
                if tx_dt >= since:
                    if tx["type"] == "+":
                        bal_in_24h  += tx.get("net", 0)
                        bal_fee_24h += tx.get("fee", 0)
                    elif tx["type"] == "-":
                        bal_out_24h += tx.get("net", 0)
                    bal_tx_24h += 1
            except Exception:
                pass
        ...
        embed.add_field(
            name="💵 Balance (kênh)",
            value=(
                f"📥 Nạp: **{fmt_amount(bal_in_24h)}**\n"
                f"📤 Chi: **{fmt_amount(bal_out_24h)}**\n"
                f"📉 Phí 5%: **{fmt_amount(bal_fee_24h)}**\n"
                f"🔢 GD: **{bal_tx_24h}** lần"
            ) if bal_tx_24h else "*(không có giao dịch)*",
            inline=True,
        )
