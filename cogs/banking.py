...
class BankingCog(commands.Cog):
    ...
    async def _notify_discord(self, tx: dict):
        ...
        direction_txt = "Tiền Vào" if is_in else "Tiền Ra"
        await send_log(
            self.bot, "BANK_TXNS", f"Giao Dịch Ngân Hàng — {direction_txt}",
            fields=[
                ("💵 Số tiền",   f"**{fmt_vnd(amount)}**", True),
                ("🏛️ Ngân hàng", tx["bank"],               True),
                ("📝 Nội dung",  tx.get("desc", "N/A")[:100] if tx.get("desc") else "N/A", False),
            ],
        )
    ...
    async def bankset_cmd(self, ctx, channel: discord.TextChannel = None):
        ...
        await send_log(
            self.bot, "SETTINGS", "Cài Kênh Log Ngân Hàng",
            fields=[
    ...
    async def slash_stats(self, interaction: discord.Interaction):
        ...
        ctx_like = type("FakeCtx", (), {
            ...
            "reply":  lambda *a, **kw: interaction.followup.send(*a, **kw),
        })()
