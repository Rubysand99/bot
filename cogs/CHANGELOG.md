...
### ✨ Thêm mới
- `cogs/admin.py` — Lệnh `.setup` (alias: `.sv_setup`, `.serversetup`) — menu setup server toàn diện với UI Discord (Buttons + Selects + Modals), chỉ dành cho admin

#### 📋 Quản lý kênh
- **Tạo kênh** — Modal nhập tên, loại (`text` / `voice` / `stage` / `forum`), public hoặc private, tuỳ chọn category đích
- **Xoá kênh** — Dropdown chọn kênh cần xoá
- **Đổi tên kênh** — Dropdown + Modal nhập tên mới (giữ icon/separator, áp font hiện tại)
- **Đổi font tên kênh** — Dropdown chọn kênh → Dropdown chọn font Unicode (bold / bold_italic / sans_bold / script / double / normal)
- **Clone kênh** — Dropdown chọn kênh, tạo bản sao với cùng cấu hình quyền

#### 🗂️ Quản lý danh mục
- **Tạo category** — Modal nhập tên + font
- **Xoá category** — Dropdown chọn category
- **Đổi tên / Font category** — Dropdown + Modal nhập tên mới và font
- **Di chuyển kênh** — Dropdown chọn kênh → Dropdown chọn category đích (hoặc không có category)

#### 🏷️ Quản lý role
- **Tạo role** — Modal nhập tên, màu hex, hoist (hiển thị riêng), mentionable
- **Xoá role** — Dropdown chọn role (kiểm tra hierarchy bot)
- **Gán role cho member** — Modal nhập User ID / mention + Role ID / tên
- **Gỡ role khỏi member** — Modal nhập User ID / mention + Role ID / tên

#### ⚙️ Setup server
- **Welcome Channel** — Dropdown chọn kênh, lưu vào `cfg_welcome_channel`
- **Goodbye Channel** — Dropdown chọn kênh, lưu vào `cfg_goodbye_channel`
- **Log Channel** — Dropdown chọn kênh, lưu vào `cfg_log_rudy`
- **Auto-role Join** — Dropdown chọn role tự động gán khi member join, lưu vào `cfg_autorole_join`
- **Đặt Prefix Bot** — Modal nhập prefix mới, lưu vào `cfg_prefix`
- **Xem trạng thái** — Embed hiển thị toàn bộ cấu hình hiện tại

### 🔧 Thay đổi
- `.help admin` — Thêm `.setup` vào danh sách lệnh
- `BOT_VERSION = "3.9.0"`
