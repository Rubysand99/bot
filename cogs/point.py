...
# Kênh log khi user bypass link & nhận mã point
CODE_GEN_LOG_CHANNEL_ID = 1504434579967316021
...
class PointCog(commands.Cog):
    ...
    async def gencode_cmd(self, ctx, *, target: str = None):
        ...
        if log_ch:
            log_embed = discord.Embed(
                title="🔑 Mã Point Vừa Được Tạo",
                color=0xF1C40F,
                timestamp=datetime.now(timezone.utc),
            )
            log_embed.add_field(name="🔑 Mã",      value=f"`{code}`",               inline=True)
            log_embed.add_field(name="💎 Point",    value=f"**{pts:,} pt**",          inline=True)
            log_embed.add_field(name="⏰ Hết hạn", value=f"**{expire_mins} phút**",  inline=True)
            log_embed.add_field(
                name="👤 Dành cho",
                value=member.mention if member else "Bất kỳ ai",
                inline=True,
            )
            log_embed.add_field(
                name="🛡️ Tạo bởi (Admin)",
                value=f"{ctx.author.mention} (`{ctx.author.id}`)",
                inline=True,
            )
            log_embed.set_footer(text="TuyTam Store  •  Point System")
            try:
                await log_ch.send(embed=log_embed)
            except Exception as e:
                print(f"[POINT] ❌ Log channel error: {e}")
    ...
    async def redeem_cmd(self, ctx, code: str = None):
        ...
        if log_ch:
            ...
            log_embed.add_field(name="🔑 Mã",       value=f"`{code}`",              inline=True)
            log_embed.add_field(name="💎 Nhận",      value=f"**+{pts:,} pt**",       inline=True)
            log_embed.add_field(name="💼 Tổng point", value=f"**{new_pts:,} pt**",   inline=True)
            log_embed.set_footer(text="TuyTam Store  •  Point System")
            try:
                await log_ch.send(embed=log_embed)
            except Exception as e:
