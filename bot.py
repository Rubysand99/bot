...
BOT_VERSION = "2.6.0"
BOT_UPDATED = "2026-05-09"

def _uname(user) -> str:
    """Trả về 'Display Name (username)' — dùng thống nhất ở mọi chỗ hiển thị tên user."""
    if user is None:
        return "Unknown"
...
class TicketButtons(View):
        ...
        try:
            for item in self.children:
                if item.custom_id == "claim_ticket":
                    item.disabled = True
                    item.label = f"Claimed: {_uname(interaction.user)}"
                    item.emoji = "✅"
                    break
    ...
    async def send_qr(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            embed.set_footer(text=f"Gửi bởi {_uname(interaction.user)}")
...
async def end_giveaway(message_id: int, channel: discord.TextChannel, winners_count: int, prize: str, host_id: int):
    ...
    embed.set_footer(text=f"Host: {_uname(host) if host else host_id}")
    await msg.edit(embed=embed, view=None)
...
async def gstart_cmd(ctx, time_str: str = None, winners: str = None, *, prize: str = None):
    ...
    embed.set_footer(text=f"Host: {_uname(ctx.author)}  •  Kết thúc lúc")
...
async def botinfo_cmd(ctx):
    ...
    embed.set_footer(
        text=f"TuyTam Store  •  Prefix: .  |  Được yêu cầu bởi {_uname(ctx.author)}",
...
async def serverinfo_cmd(ctx):
    ...
    embed.set_footer(
        text=f"TuyTam Store  •  Được yêu cầu bởi {_uname(ctx.author)}",
        icon_url=ctx.author.display_avatar.url
    )
    await ctx.reply(embed=embed)
...
async def done_cmd(ctx, amount_str: str = None):
    ...
    embed.set_footer(text=f"Xác nhận bởi {_uname(ctx.author)}")
    await ctx.reply(embed=embed)
...
async def giveaway_timer(channel_id: int, message_id: int, winners_count: int, seconds: int):
    ...
    embed.set_footer(text=f"Host: {_uname(host) if host else gw['host']}")
    await msg.edit(embed=embed, view=None)
    await channel.send(f"🎊 Chúc mừng {winner_mentions}! Bạn đã thắng **{gw['prize']}**!")

    gw["winner_ids"] = winner_ids
    save_giveaways_data()   # ← lưu kết quả lên Discord
...
async def handle_balance_message(message: discord.Message):
    ...
    if op == "+":
        ...
        embed.set_footer(text=f"Bởi {_uname(message.author)}  •  {now_str}")
...
class BuyerSpendingView(View):
    ...
    def _build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(
            text=f"Trang {self.page+1}/{total_pages}  •  Yêu cầu bởi {_uname(self.requester)}"
        )
        return embed
...
async def ai_cmd(ctx, *, prompt: str = None):
    ...
    if sub == "tomtat":
        ...
        async with ctx.typing():
            ...
            chat_log = "\n".join(f"{_uname(m.author)}: {m.content}" for m in msgs if m.content)[:4000]
            task = (
        ...
        embed.set_footer(text=f"Yêu cầu bởi {_uname(ctx.author)}")
        return await ctx.reply(embed=embed)
    ...
    if sub == "phantich":
        ...
        async with ctx.typing():
            ...
            task = (
                f"Phân tích phong cách chat của người dùng Discord tên '{_uname(target)}' "
...
async def mkchannel_cmd(ctx, *, args: str = None):
    ...
    try:
        ...
        embed.set_footer(text=f"Tạo bởi {_uname(ctx.author)}")
        await ctx.reply(embed=embed)
...
async def on_ready():
    ...
    if changelog_ch:
        ...
        embed.add_field(
            name="🆕 v2.6.0 — Invite Tracking",
            value=(
                "• Bot tự động theo dõi số lần invite của từng user\n"
                "• Phát hiện **fake invite** (join rồi leave trong 10 phút)\n"
                "• Lệnh `.invite [@user]` — xem số invite\n"
                "• Lệnh `.invitetop [n]` — bảng xếp hạng invite\n"
                "• Lệnh `.resetinvite @user / all` — reset invite *(admin)*\n"
                "• Dữ liệu lưu riêng trong MongoDB, không ảnh hưởng giveaway/buyer/ticket"
            ),
            inline=False
        )
        embed.add_field(
            name="📦 v2.5.0 — Font & Tạo Kênh",
            value=(
                "• `.mkchannel <text/voice/category> <tên>` — tạo kênh đồng bộ font server\n"
                "• Font server tự động lưu sau khi rename hàng loạt"
            ),
            inline=False
        )
        embed.add_field(
            name="🤖 v2.4.0 — AI Commands",
            value=(
                "• `.ai` — chat AI dùng mọi kênh\n"
                "• `.ai tomtat` / `.ai dich` / `.ai phantich`\n"
                "• Kênh AI tự động trả lời cài qua `.settings`"
            ),
            inline=False
        )
