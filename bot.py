...
# Chỉ OWNER mới dùng được các lệnh nguy hiểm (xoá kênh, reset data, setup server...)
OWNER_ID = 846332174734983219
...
TRANSCRIPT_CHANNEL_ID = 1464430574524436679  # kênh gửi transcript ticket
FEEDBACK_CHANNEL_ID   = 1502464872686948403  # kênh gửi feedback/rating ticket
...
def get_sellers() -> list:
    """Trả về danh sách seller từ MongoDB. Mỗi phần tử: {id, label}"""
    data = load_data()
...
def _default_data() -> dict:
    return {
        ...
        "ratings":          [],
        "ticket_notes":     {},
        # invite_counts: { "user_id": { "total": int, "fake": int, "left": int } }
        # — "total"  = tổng lần invite thực sự join (đã xác minh qua snapshot)
        # — "fake"   = member join rồi leave trong 10 phút (không tính)
        # — "left"   = member đã rời server (trừ khỏi net count)
        # net invite = total - fake - left
        "invite_counts":    {},
    }
...
def load_data() -> dict:
    """Đọc từ cache in-memory."""
    if _data_cache is not None:
        return _data_cache
    return _default_data()

def save_data(data: dict):
    """Lưu vào cache và flush lên MongoDB."""
    global _data_cache
    _data_cache = data
    try:
...
async def init_data_cache():
    """Gọi khi bot ready — tải data từ MongoDB."""
    global _data_cache
    _data_cache = await _mongo_load()
    # Load giveaways từ MongoDB vào cache luôn
...
def _load_giveaway_section() -> dict:
    """Đọc giveaways từ cache (đã load trong init)."""
    data = load_data()
    return data.get("_giveaways", {})

def _save_giveaway_section(giveaways: dict):
    """Lưu giveaways vào cache và MongoDB."""
    serializable = {}
    ...
    save_data(data)
    # Đồng thời lưu từng giveaway vào collection riêng
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
...
async def _sync_giveaways_to_mongo(giveaways: dict):
    """Ghi tất cả giveaways lên collection MongoDB."""
    _, col_gw = _get_mongo()
    try:
...
def fmt_amount(amount: int) -> str:
    """Format số tiền dễ đọc. VD: 50000 → '50k', 1500000 → '1.5tr'"""
    if amount >= 1_000_000:
...
async def create_order_ticket(interaction: discord.Interaction, trade_type: str,
    ...
    try:
        ...
        # Tên ticket = tên seller + số ticket, ví dụ: tuytam-001
        safe_seller = seller_name.lower().replace(" ", "").replace("_", "")[:20]
        channel_name = f"{safe_seller}-{number}"
        ...
        # KHÔNG cấp theo role — chỉ cấp đúng seller được chọn (theo ID cá nhân)
        if seller_id:
        ...
        channel  = await guild.create_text_channel(
            ...
            overwrites=overwrites,
            category=category,
            topic=f"{interaction.user.id}||{trade_type}|{seller_name}|open"
        )

        seller_display = f"<@{seller_id}>" if seller_id else seller_name

        embed = discord.Embed(
            title=f"{type_label}  •  {seller_name}  •  #{number}",
            description=(
                f"Xin chào {interaction.user.mention}! 👋\n"
                f"Staff sẽ xử lý giao dịch sớm nhất có thể.\n"
                f"🟡 **Trạng thái:** Đang chờ staff nhận"
...
class SellerSelect(Select):
    def __init__(self, trade_type: str):
        ...
        if sellers:
            ...
            # Thêm lựa chọn random ở cuối
            options.append(
                discord.SelectOption(
                    label="🎲 Random (tự chọn seller ngẫu nhiên)",
                    value="random",
                    description="Bot sẽ tự chọn 1 seller ngẫu nhiên cho bạn"
                )
            )
        else:
            options = [
                discord.SelectOption(label="⚠️ Chưa có seller — liên hệ admin", value="0")
            ]
        action = "mua" if trade_type == "sell" else "bán"
        super().__init__(
            placeholder=f"Chọn seller bạn muốn {action}...",
            options=options,
            custom_id=f"seller_select_{trade_type}"
        )

    async def callback(self, interaction: discord.Interaction):
        import random as _random
        try:
            seller_id_str = self.values[0]
            if seller_id_str == "0":
                return await interaction.response.send_message(
                    "❌ Chưa có seller nào! Admin dùng `.addseller <ID> <tên>` để thêm.",
                    ephemeral=True
                )
            sellers = get_sellers()
            # Xử lý random
            if seller_id_str == "random":
                if not sellers:
                    return await interaction.response.send_message(
                        "❌ Chưa có seller nào để random!", ephemeral=True
                    )
                chosen = _random.choice(sellers)
                seller_id   = chosen["id"]
                seller_name = chosen["label"]
            else:
                seller_id   = int(seller_id_str)
                seller_name = next((s["label"] for s in sellers if s["id"] == seller_id), str(seller_id))
...
class SellerSelectView(View):
    def __init__(self, trade_type: str):
        super().__init__(timeout=60)
        self.add_item(SellerSelect(trade_type))
...
class TicketPanel(View):
    ...
    async def buy(self, interaction: discord.Interaction, button: Button):
        try:
            await interaction.response.send_message(
                "🛒 **Chọn seller bạn muốn mua:**",
                view=SellerSelectView(trade_type="sell"),
                ephemeral=True
            )
        except Exception as e:
            try:
    ...
    async def sell(self, interaction: discord.Interaction, button: Button):
        try:
            await interaction.response.send_message(
                "💸 **Chọn seller bạn muốn bán:**",
                view=SellerSelectView(trade_type="buy"),
                ephemeral=True
            )
        except Exception as e:
            try:
...
async def _complete_order(interaction: discord.Interaction):
    ...
    # Yêu cầu nhập số tiền qua modal
...
async def _close_ticket(channel, bot_instance, closer: discord.Member = None):
    ...
    # ── Gửi embed log chi tiết + transcript vào kênh transcript riêng ──
    ...
    # Gửi transcript (embed + file HTML) vào kênh transcript
    if transcript_ch:
    ...
    # ── Ghi chú nội bộ → gửi vào kênh transcript ──
...
async def help_cmd(ctx, category: str = None):
    if category is None:
        ...
        cats = [
            ...
            ("🛡️ `.help mod`",      "`.clear` `.addrole` `.removerole` `.mkchannel` `.rename` `.emoji`"),
...
def save_giveaways_data():
    """Lưu active_giveaways lên section GIVEAWAY trên Discord."""
    _save_giveaway_section(active_giveaways)

def load_giveaways_data() -> dict:
    """Đọc giveaways từ MongoDB, trả về dict với entries là set."""
...
async def end_giveaway(message_id: int, channel: discord.TextChannel, winners_count: int, prize: str, host_id: int):
    """Kết thúc giveaway dùng .gstart (reaction 🎉)."""
    try:
...
async def gstart_cmd(ctx, time_str: str = None, winners: str = None, *, prize: str = None):
    ...
    save_giveaways_data()   # ← lưu ngay để sống sót qua restart
...
async def greroll_cmd(ctx, message_id: int = None):
    ...
    try:
        ...
        # Cũng thử lấy từ entries (slash giveaway)
...
class SettingsView(View):
    ...
    async def dangerous_cmds(self, interaction: discord.Interaction, button: Button):
        if interaction.user.id != OWNER_ID:
            return await interaction.response.send_message(
                "❌ Chỉ owner mới chỉnh được phân quyền lệnh nguy hiểm.", ephemeral=True
            )
...
def get_dangerous_overrides() -> dict:
    """Lấy cấu hình override từ data. Mặc định tất cả là owner_only."""
...
def can_use_dangerous_cmd(user_id: int, cmd_name: str) -> bool:
    """Kiểm tra user có quyền dùng lệnh nguy hiểm không."""
    if user_id == OWNER_ID:
        return True
    overrides = get_dangerous_overrides()
    return overrides.get(cmd_name) == "admin" and user_id in ADMIN_IDS
...
class DangerousCommandsView(View):
    ...
    def _make_callback(self, cmd: str):
        async def callback(interaction: discord.Interaction):
            if interaction.user.id != OWNER_ID:
                return await interaction.response.send_message("❌ Chỉ owner mới chỉnh được.", ephemeral=True)
            overrides = get_dangerous_overrides()
...
async def done_cmd(ctx, amount_str: str = None):
    ...
    # Chống gọi 2 lần trong cùng 1 ticket
    data = load_data()
...
class GiveawayView(View):
    ...
    async def join(self, interaction: discord.Interaction, button: Button):
        # Lấy message_id từ interaction (đúng cả sau khi bot restart)
        ...
        # Lưu lại entries mới vào MongoDB
        save_giveaways_data()
...
async def giveaway_timer(channel_id: int, message_id: int, winners_count: int, seconds: int):
    ...
    # ── Kiểm tra invite count của winner nếu được bật ──
...
class GiveawayModal(discord.ui.Modal, title="🎉 Tạo Giveaway"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Gửi confirm view (ephemeral) cho admin trước khi đăng
...
class GiveawayConfirmView(View):
    ...
    def _update_button_label(self):
        """Cập nhật label nút toggle theo trạng thái hiện tại."""
        for item in self.children:
    ...
    async def confirm(self, interaction: discord.Interaction, button: Button):
        ...
        active_giveaways[msg.id] = {
            ...
            "host":        self.host.id,
            "ended":       False,
            "send_invite": self.send_invite,   # ← lưu trạng thái gửi invite
        }
        save_giveaways_data()
...
async def _call_groq(user_id: int, user_message: str) -> str:
    """Gọi Groq API với fallback tự động khi hết quota."""
    if not GROQ_API_KEY:
        ...
        try:
            async with _aiohttp_ai.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 429:
                        # Hết quota model này → thử model tiếp theo
...
async def handle_ai_message(message: discord.Message):
    """Xử lý tin nhắn trong kênh AI chat."""
    ai_ch_id = get_cfg_ai_channel()
...
async def ai_reset(ctx):
    """Xoá lịch sử hội thoại AI của tất cả user. Chỉ admin."""
    if ctx.author.id not in ADMIN_IDS:
...
async def my_chat_reset(ctx):
    """User tự xoá lịch sử chat AI của mình."""
...
async def invite_cmd(ctx, member: discord.Member = None):
    """Xem số lần invite của bản thân hoặc user khác."""
...
async def invitetop_cmd(ctx, top: int = 10):
    """Bảng xếp hạng invite top N (mặc định 10). *(admin)*"""
    if ctx.author.id not in ADMIN_IDS:
        return await ctx.reply("❌ Chỉ admin.")
...
async def mkchannel_cmd(ctx, *, args: str = None):
    ...
    # Áp dụng font server vào tên kênh
    ...
    # Tìm category mặc định (ticket category hoặc None)
...
async def _cache_invites(guild: discord.Guild):
    """Lấy snapshot invite hiện tại của guild vào cache."""
    try:
...
def _add_invite(inviter_id: int, field: str, amount: int = 1):
    """Tăng field (total/fake/left) cho inviter_id trong MongoDB."""
    counts = _get_invite_counts()
    uid = str(inviter_id)
...
def _get_net_invites(inviter_id: int) -> tuple[int, int, int, int]:
    """Trả về (total, fake, left, net) của inviter."""
    counts = _get_invite_counts()
    uid = str(inviter_id)
...
async def on_member_join(member: discord.Member):
    ...
    # ── 2. Xác định ai đã invite member này ──
    try:
        ...
        # Cập nhật cache với snapshot mới
        _invite_cache[member.guild.id] = new_cache

        if inviter_id and inviter_id != member.id:
            # Ghi nhận join vào pending (chờ 10 phút kiểm tra fake)
            ...
            # Cộng invite tạm thời — nếu fake sẽ bị trừ sau
            _add_invite(inviter_id, "total", 1)

            # Sau 10 phút kiểm tra còn trong server không
            async def _check_fake():
                await asyncio.sleep(600)  # 10 phút
                still_here = member.guild.get_member(member.id)
                if not still_here:
                    # Member đã leave trong 10 phút → fake invite
...
async def on_member_remove(member: discord.Member):
    # Cập nhật cache invite sau khi member leave
    await _cache_invites(member.guild)

    # Nếu member đã ở server > 10 phút và đã được ai invite → tăng left
    pend = _pending_joins.pop(member.id, None)
    if pend is None:
        # Không còn pending nghĩa là đã qua 10 phút — tính là "left"
        # Tìm xem ai đã invite member này (từ invite_counts không thể biết)
        # → chỉ tính left nếu member từng được track
        # LƯU Ý: không thể xác định inviter sau khi hết pending
        # nên không cộng left để tránh sai data
        pass
    # Nếu còn pending (leave trong 10 phút) → _check_fake sẽ xử lý
    # Cập nhật snapshot invite cache
    await _cache_invites(member.guild)
...
async def _show_buy_role_panel(interaction: discord.Interaction):
    """Hiển thị panel quản lý role buyer tự động theo khoảng tiền."""
    buy_roles = get_buy_roles()
    embed = discord.Embed(
...
class RenamePreviewView(View):
    ...
    async def btn_confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Lưu font server sau khi apply thành công
        if done > 0:
...
_DEFAULT_PRICE_SECTIONS = [
    {
        ...
        "content": (
            ...
            "> - **acc online hỏi <@846332174734983219>**"
        ),
    },
    {
        "key": "robux",
...
def get_price_sections() -> list:
    """Lấy bảng giá từ data (MongoDB). Nếu chưa có → dùng default."""
    data = load_data()
...
def build_sv_embed() -> discord.Embed:
    """Build embed bảng giá động từ data."""
    embed = discord.Embed(
...
async def sv_command(ctx: commands.Context):
    """Gửi embed bảng giá TuyTam Store."""
...
async def giaset_command(ctx: commands.Context):
    """Admin quản lý bảng giá .sv — sửa/thêm/xoá mục."""
    if ctx.author.id not in ADMIN_IDS:
        return await ctx.reply("❌ Bạn không có quyền dùng lệnh này.")
...
async def resume_active_giveaways():
    """Đọc giveaways đã lưu và tiếp tục đếm ngược các giveaway chưa kết thúc."""
    ...
    for mid, gw in saved.items():
        ...
        if remaining <= 0:
            # Đã quá giờ khi bot offline → kết thúc ngay
            channel = bot.get_channel(channel_id)
            if channel:
                if gw_type == "button":
...
async def on_ready():
    ...
    # ── Bước 2: Resume giveaway chưa kết thúc (sống sót qua restart) ──
    ...
    # ── Bước 3: Cache snapshot invites của tất cả guild ──
    ...
    # ── Bước 4: Gửi changelog vào kênh chỉ định ──
