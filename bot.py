...
async def gpick_cmd(ctx, message_id: int = None, *members: discord.Member):
    """
    Admin chỉ định winner cho giveaway thay vì random.
    Dùng: .gpick <message_id> @user1 @user2 ...
    ...
    if not message_id:
        return await ctx.reply(
            "❌ Dùng: `.gpick <message_id> @user1 @user2 ...`\n"
            "Ví dụ: `.gpick 1234567890 @TuyTam`"
        )
    if not members:
        return await ctx.reply("❌ Phải mention ít nhất 1 người thắng!\nVí dụ: `.gpick 1234567890 @TuyTam @SomeOne`")
