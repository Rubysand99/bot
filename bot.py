...
async def _run_live_watcher(username: str):
    ...
    while True:
        ...
        async def on_connect(event: ConnectEvent):
            ...
            # Lấy thumbnail live từ TikTokLive client nếu có
            live_thumb = None
            try:
                room = getattr(client, "room_info", None) or {}
                # Một số version TikTokLive lưu cover ở room_info
                live_thumb = (
                    room.get("cover", {}).get("url_list", [None])[0]
                    or room.get("stream_url", {}).get("flv_pull_url", {}).get("HD1", None)
                )
            except Exception:
                pass
