...
TIKTOK_LIVE_RETRY_INTERVAL  = 30   # giây — thử reconnect LIVE client sau lỗi
...
async def _run_live_watcher(username: str):
    ...
    - Gửi thông báo Discord khi LIVE bắt đầu (ConnectEvent).
    - Tự động reconnect sau khi stream kết thúc hoặc lỗi.
    """
    try:
    ...
    while True:
        ...
        async def on_connect(event: ConnectEvent):
            """Kích hoạt khi @username bắt đầu LIVE."""
            ...
            try:
                ...
                print(f"[TikTok-LIVE] ❌ Gửi thông báo LIVE lỗi: {e}")

        @client.on(DisconnectEvent)
        async def on_disconnect(event: DisconnectEvent):
            """Kích hoạt khi stream kết thúc."""
        ...
        try:
            # connect() chờ cho đến khi stream kết thúc hoặc lỗi
            # Nếu user chưa live, nó sẽ raise exception ngay
            await client.connect()
        except Exception as e:
            err_msg = str(e).lower()
            if "not live" in err_msg or "offline" in err_msg or "could not find" in err_msg:
                # User chưa live — thử lại sau
                pass
            else:
                print(f"[TikTok-LIVE] ⚠️ Lỗi kết nối @{username}: {e}")

        # Dù live hay chưa, đợi rồi thử lại để phát hiện lần live tiếp theo
