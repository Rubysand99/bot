...
def _detect_channel_parts(name: str):
    ...
    - separator (• hoặc - hoặc khoảng trắng)
    - base_text (phần chữ chính)
    - trailing_number (số cuối nếu có, vd: legit-58)
    Trả về dict.
    """
    # Tìm icon ở đầu (emoji unicode hoặc custom)
    ...
    # Tìm số ở cuối
    num_match = _re_setup.search(r'[\-–](\d+)$', rest)
    trailing_num = ""
    base_text = rest
    if num_match:
        trailing_num = num_match.group(0)   # vd: "-58"
        base_text = rest[:num_match.start()]
...
class CreateChannelModal(Modal, title="➕ Tạo Kênh Mới"):
    ...
    font    = TextInput(label="Font chữ (normal/bold/sans_bold/script/double)", placeholder="normal", default="normal", max_length=20, required=False)
...
class SetupFontSampleModal(Modal, title="✏️ Đổi Tên Kênh Hàng Loạt"):
    sample = TextInput(
        label="Chuỗi mẫu font (copy từ Discord hoặc gõ tên)",
        placeholder="Paste mẫu vd: 𝐐𝐮𝐢𝐞𝐭 𝐒𝐜𝐡𝐞𝐦𝐚𝐭𝐢𝐜𝐬  hoặc gõ: bold",
        max_length=50,
        required=False
    )
    font_name = TextInput(
        label="Hoặc gõ tên font: normal/bold/sans_bold/script/double",
        placeholder="bold  (= font 𝐐𝐮𝐢𝐞𝐭 𝐒𝐜𝐡𝐞𝐦𝐚𝐭𝐢𝐜𝐬)",
        default="bold",
        max_length=20,
        required=False
    )
...
def _detect_font_from_sample(sample: str) -> str:
    ...
    # Kiểm tra alias trước (user gõ tên như "bold", "quiet", "sb"...)
...
class RenamePreviewView(View):
    ...
    async def btn_preview(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not lines:
            return await interaction.response.send_message("✅ Không có kênh nào cần đổi tên với font này.", ephemeral=True)
        text = "\n".join(lines[:25])
        await interaction.response.send_message(
    ...
    async def btn_confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        channels = self._get_channels()
        await interaction.response.send_message(
            f"⏳ Đang đổi tên **{len(channels)}** kênh...", ephemeral=True
        )
