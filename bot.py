...
# ================= SELECT SELLER =================
def _get_seller_status(guild: discord.Guild, seller_id: int) -> str:
    """Trả về emoji trạng thái online của seller."""
...
class SellerSelect(Select):
    def __init__(self, trade_type: str, guild: discord.Guild):
        self.trade_type = trade_type
        sellers = get_sellers()
        if sellers:
            options = []
            for s in sellers:
                status = _get_seller_status(guild, s["id"])
                member = guild.get_member(s["id"])
                status_text = {
                    "🟢": "Online",
                    "🟡": "Idle",
                    "🔴": "Bận (DND)",
...
class SellerSelectView(View):
    def __init__(self, trade_type: str, guild: discord.Guild):
        ...
        self.add_item(SellerSelect(trade_type, guild))
    ...
    async def buy(self, interaction: discord.Interaction, button: Button):
        try:
            await interaction.response.send_message(
                ...
                view=SellerSelectView(trade_type="sell", guild=interaction.guild),
                ephemeral=True
            )
        except Exception as e:
            try:
    ...
    async def sell(self, interaction: discord.Interaction, button: Button):
        try:
            await interaction.response.send_message(
                ...
                view=SellerSelectView(trade_type="buy", guild=interaction.guild),
                ephemeral=True
            )
        except Exception as e:
            try:
...
async def delemoji_cmd(ctx, *, args: str = None):
    ...
    # ── Tìm emoji theo custom emoji format <:name:id> hoặc <a:name:id> ──
    _emoji_pattern = r"<(a?):([^:>]+):(\d+)>"
    matches = _re_emoji.findall(_emoji_pattern, args)
