...
SELLER_ROLE_ID  = 0   # ← Thay bằng ID role Seller của bạn (hoặc cài qua .settings)
COUNTER_CHANNEL_ID = 0  # ← Thay bằng ID kênh lưu counter của bạn
LEGIT_CHANNEL_ID = 0   # ← Thay bằng ID kênh ✅•𝐋𝐞𝐠𝐢𝐭-XX của bạn
PROOF_CHANNEL_ID = 1469647159560241318  # kênh proof/vouch — tự động +1 khi gửi "done"
...
def is_staff(member: discord.Member) -> bool:
    """Trả về True nếu member là admin, có role support, hoặc có role seller."""
...
DEFAULT_DATA = {
    ...
    # buy_roles: list các dict {"min_orders": int, "role_id": int, "label": str}
    # Ví dụ: [{"min_orders": 1, "role_id": 123, "label": "buy+"}, {"min_orders": 5, "role_id": 456, "label": "buy++"}]
    "buy_roles": [],
    # Lưu số đơn đã mua của từng user: {"user_id_str": int}
    "user_order_count": {},
}
...
def get_buy_roles() -> list:
    """Trả về danh sách cấu hình role theo số đơn. Đã sắp xếp tăng dần min_orders."""
    data = load_data()
    roles = data.get("buy_roles", [])
    return sorted(roles, key=lambda r: r.get("min_orders", 0))
...
def get_user_order_count(user_id: int) -> int:
    data = load_data()
    return data.get("user_order_count", {}).get(str(user_id), 0)

def increment_user_order_count(user_id: int) -> int:
    """Tăng số đơn của user lên 1 và trả về số mới."""
    data = load_data()
    if "user_order_count" not in data:
        data["user_order_count"] = {}
    key = str(user_id)
    data["user_order_count"][key] = data["user_order_count"].get(key, 0) + 1
    save_data(data)
    return data["user_order_count"][key]

async def auto_give_buy_roles(guild: discord.Guild, member: discord.Member, order_count: int):
    """
    Tự động give/upgrade role mua hàng cho member dựa vào số đơn.
    Logic: Give tất cả role mà member đủ điều kiện (min_orders <= order_count).
    Xoá các role cũ hơn nếu có role cao hơn.
    """
    buy_roles = get_buy_roles()
    if not buy_roles:
        return

    # Xác định các role mà user đủ điều kiện
    eligible = [r for r in buy_roles if order_count >= r.get("min_orders", 0)]
    # Role cao nhất (min_orders lớn nhất) mà user đủ điều kiện
    ...
    for cfg in buy_roles:
        ...
        if not role:
            continue
        if highest and cfg["role_id"] == highest["role_id"]:
            # Give role cao nhất
            if role not in member.roles:
                try:
                    await member.add_roles(role, reason=f"Auto buy role — {order_count} đơn")
                except Exception as e:
                    print(f"[BUY_ROLE] Lỗi give role {role.name}: {e}")
        else:
            # Xoá các role buy thấp hơn (nếu có)
            if role in member.roles:
                try:
                    await member.remove_roles(role, reason=f"Upgrade role — {order_count} đơn")
                except Exception as e:
                    print(f"[BUY_ROLE] Lỗi xoá role {role.name}: {e}")
    ...
    def __init__(self, trade_type: str):
        ...
        options = [
            discord.SelectOption(
                label=info["label"],
                value=key,
                description=info["note"] if info["note"] else f"Đơn vị: {info['unit']}",
...
async def _complete_order(interaction: discord.Interaction):
    ...
    # Chỉ give role cho đơn mua/bán thực sự
    ...
    # Kiểm tra đơn đã được đánh dấu hoàn thành chưa (tránh nhấn 2 lần)
    data = load_data()
    completed_key = f"completed_{channel.id}"
    if data.get(completed_key):
        return await interaction.followup.send(
            f"⚠️ Đơn này đã được đánh dấu hoàn thành rồi!\n"
            f"Buyer: {buyer.mention} — **{get_user_order_count(user_id)}** đơn tổng cộng.",
            ephemeral=True
        )
    ...
    # +1 đơn và give role
    new_count = increment_user_order_count(user_id)
    highest_role_cfg = await auto_give_buy_roles(guild, buyer, new_count)

    # Vô hiệu hoá nút để tránh nhấn lại
    try:
        view = interaction.message.view if interaction.message else None
        if view:
            for item in view.children:
                if getattr(item, "custom_id", None) == "complete_order":
                    item.disabled = True
                    item.label = f"Đã hoàn thành ({new_count} đơn)"
                    break
    ...
    buy_roles = get_buy_roles()
    if highest_role_cfg:
        role_obj = guild.get_role(highest_role_cfg.get("role_id", 0))
        role_mention = role_obj.mention if role_obj else f"**{highest_role_cfg.get('label','?')}**"
        result_msg = (
            f"🏆 **Hoàn thành đơn!**\n"
            f"👤 Buyer: {buyer.mention}\n"
            f"📦 Tổng đơn: **{new_count}**\n"
            f"🎭 Role nhận được: {role_mention}"
        )
    else:
        # Chưa đủ role nào
        if buy_roles:
            next_role = buy_roles[0]
            need = next_role.get("min_orders", 1) - new_count
            result_msg = (
                f"✅ **Đã ghi nhận đơn!**\n"
                f"👤 Buyer: {buyer.mention}\n"
                f"📦 Tổng đơn: **{new_count}**\n"
                f"⏳ Cần thêm **{need}** đơn để nhận role **{next_role.get('label','buy+')}**"
            )
        else:
            result_msg = (
                f"✅ **Đã ghi nhận đơn!**\n"
                f"👤 Buyer: {buyer.mention}\n"
                f"📦 Tổng đơn: **{new_count}**\n"
                f"⚙️ Chưa có role nào được cấu hình — dùng `.setup` → 🏆 Cài Role Mua Hàng"
            )

    # Gửi thông báo public trong ticket
    await channel.send(result_msg)
    await interaction.followup.send("✅ Đã xử lý xong.", ephemeral=True)
...
HELP_CATEGORIES = {
    ...
    "kenh": {
        "title": "📌  ID Kênh Quan Trọng",
        "fields": [
            ("Data Channel",    f"`{DATA_CHANNEL_ID}` — lưu toàn bộ data JSON bot"),
            ("Proof Channel",   f"`{PROOF_CHANNEL_ID}` — gõ `done` để +1 số đơn"),
            ("Legit Channel",   "ID cài qua `.settings` → ✅ Legit Channel — gõ `+1legit` để +1"),
            ("Balance Channel", "ID cài qua `.settings` → 💰 Balance Channel — gõ `+` / `-` để nạp/chi"),
            ("Counter Channel", "ID cài qua `.settings` → 🔢 Counter Channel — lưu số ticket"),
            ("Log Channel",     "ID cài qua `.settings` → log ticket đóng"),
        ]
    },
}
...
async def help_cmd(ctx, category: str = None):
    if category is None:
        ...
        cats = [
            ...
            ("🎫 `.help ticket`",   "`.panel` `.done` `.close` `.addnote` `.ratings` `.orderbase`"),
            ...
            ("📌 `.help kenh`",     "Data, Proof, Legit, Balance, Counter, Log channel IDs"),
        ]
...
async def settings(ctx):
    ...
    if buy_roles:
        buy_role_val = "\n".join(
            f"› **{r.get('label','?')}** — từ {r.get('min_orders',0)} đơn"
            for r in buy_roles
        )
    else:
...
async def done_cmd(ctx):
    """Đánh dấu hoàn thành đơn hàng trong ticket: +1 đơn cho buyer và give role tự động."""
    if not is_staff(ctx.author):
        return await ctx.reply("❌ Bạn không có quyền.")
    if not (ctx.channel.topic and "|" in ctx.channel.topic):
        return await ctx.reply("❌ Đây không phải kênh ticket.")
    ...
    if data.get(completed_key):
        ...
        return await ctx.reply(
            f"⚠️ Đơn này đã được đánh dấu hoàn thành rồi!\n"
            f"Buyer: {buyer.mention} — **{count}** đơn tổng cộng."
        )
    ...
    # +1 đơn và give role
    new_count = increment_user_order_count(user_id)
    highest_role_cfg = await auto_give_buy_roles(ctx.guild, buyer, new_count)

    buy_roles = get_buy_roles()
    if highest_role_cfg:
        role_obj = ctx.guild.get_role(highest_role_cfg.get("role_id", 0))
        role_mention = role_obj.mention if role_obj else f"**{highest_role_cfg.get('label','?')}**"
        msg = (
            f"🏆 **Hoàn thành đơn!**\n"
            f"👤 Buyer: {buyer.mention}\n"
...
async def _show_buy_role_panel(interaction: discord.Interaction):
    """Hiển thị panel quản lý role mua hàng tự động."""
    buy_roles = get_buy_roles()
    embed = discord.Embed(
        title="🏆  Cấu Hình Role Mua Hàng Tự Động",
        description=(
            "Bot sẽ tự động give role cho người mua khi staff gõ `done` trong kênh proof.\n"
            "Mỗi role ứng với một mốc số đơn nhất định.\n\n"
            "**Cách hoạt động:**\n"
            "› Staff gõ `done` (có thể mention @buyer hoặc reply tin nhắn buyer)\n"
            "› Bot +1 đơn cho buyer, kiểm tra mốc → give role cao nhất phù hợp\n"
            "› Role thấp hơn bị tự động xoá (upgrade role)"
        ),
        color=0xF1C40F,
        timestamp=datetime.now(timezone.utc)
    )
    if buy_roles:
        ...
        for i, r in enumerate(buy_roles):
            ...
            lines.append(f"`{i+1}.` {role_str} — **{r.get('label','?')}** — từ **{r.get('min_orders',0)}** đơn")
    ...
    embed.set_footer(text="TuyTam Store  •  Setup → Role Mua Hàng")
...
class BuyRoleManageView(View):
    ...
    async def btn_clear(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in ADMIN_IDS:
            return await interaction.response.send_message("❌ Chỉ admin.", ephemeral=True)
        save_buy_roles([])
        await interaction.response.send_message("✅ Đã xoá toàn bộ cấu hình role mua hàng.", ephemeral=True)

    @discord.ui.button(label="📊 Xem số đơn user", style=discord.ButtonStyle.secondary, row=1)
    async def btn_orders(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in ADMIN_IDS:
            return await interaction.response.send_message("❌ Chỉ admin.", ephemeral=True)
        await interaction.response.send_modal(CheckUserOrdersModal())

    @discord.ui.button(label="✏️ Sửa số đơn user", style=discord.ButtonStyle.secondary, row=1)
    async def btn_set_orders(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in ADMIN_IDS:
            return await interaction.response.send_message("❌ Chỉ admin.", ephemeral=True)
        await interaction.response.send_modal(SetUserOrdersModal())
    ...
    async def btn_sync(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Scan toàn bộ member có buy role → gợi ý set số đơn mặc định, không ghi đè ai đã có data."""
        if interaction.user.id not in ADMIN_IDS:
            return await interaction.response.send_message("❌ Chỉ admin.", ephemeral=True)
        await interaction.response.send_modal(SyncExistingRolesModal())


class AddBuyRoleModal(Modal, title="➕ Thêm Role Mua Hàng"):
    role_id_input = TextInput(
        label="ID của Role",
        placeholder="Chuột phải vào role → Copy ID",
        min_length=15, max_length=20
    )
    min_orders_input = TextInput(
        label="Số đơn tối thiểu để nhận role",
        placeholder="Ví dụ: 1 (buy+), 5 (buy++), 10 (buy+++)",
        min_length=1, max_length=5
    )
    label_input = TextInput(
        label="Tên hiển thị của role",
        placeholder="Ví dụ: buy+, buy++, buy+++",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            min_ord   = int(self.min_orders_input.value.strip())
            label     = self.label_input.value.strip()
        except ValueError:
            return await interaction.response.send_message("❌ ID role hoặc số đơn không hợp lệ!", ephemeral=True)

        role_obj = interaction.guild.get_role(role_id)
        if not role_obj:
            return await interaction.response.send_message(
                f"❌ Không tìm thấy role với ID `{role_id}` trong server này.", ephemeral=True
            )

        buy_roles = get_buy_roles()
        # Kiểm tra trùng role_id
        for r in buy_roles:
            if r["role_id"] == role_id:
                return await interaction.response.send_message(
                    f"❌ Role {role_obj.mention} đã được cấu hình rồi (mốc {r['min_orders']} đơn).", ephemeral=True
                )

        buy_roles.append({"role_id": role_id, "min_orders": min_ord, "label": label})
        buy_roles = sorted(buy_roles, key=lambda r: r["min_orders"])
        save_buy_roles(buy_roles)

        embed = discord.Embed(
            title="✅ Đã Thêm Role Mua Hàng",
            color=0x57F287,
            timestamp=datetime.now(timezone.utc)
        )
        embed.add_field(name="🏷️ Role",           value=role_obj.mention,  inline=True)
        embed.add_field(name="📛 Tên",             value=f"`{label}`",      inline=True)
        embed.add_field(name="🔢 Số đơn tối thiểu", value=f"**{min_ord}** đơn", inline=True)

        all_lines = []
        for r in buy_roles:
            ro = interaction.guild.get_role(r["role_id"])
            all_lines.append(f"› {ro.mention if ro else r['role_id']} — **{r['label']}** — từ **{r['min_orders']}** đơn")
...
class RemoveBuyRoleModal(Modal, title="🗑️ Xoá Role Mua Hàng"):
    role_id_input = TextInput(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        role_obj = interaction.guild.get_role(role_id)
        await interaction.response.send_message(
            f"✅ Đã xoá role **{role_obj.name if role_obj else role_id}** khỏi danh sách mua hàng.",
            ephemeral=True
        )


class CheckUserOrdersModal(Modal, title="📊 Xem Số Đơn Của User"):
    user_id_input = TextInput(
        label="ID của User",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        member = interaction.guild.get_member(uid)
        count  = get_user_order_count(uid)
        buy_roles = get_buy_roles()
        ...
        embed.add_field(name="📦 Số đơn", value=f"**{count}** đơn",                         inline=True)

        # Role hiện tại
        current_roles = [r for r in buy_roles if count >= r.get("min_orders", 0)]
        highest = current_roles[-1] if current_roles else None
        if highest:
            ro = interaction.guild.get_role(highest["role_id"])
            embed.add_field(name="🏆 Role hiện tại", value=ro.mention if ro else highest["label"], inline=True)
        else:
            embed.add_field(name="🏆 Role hiện tại", value="Chưa đủ điều kiện", inline=True)

        # Role tiếp theo
        next_roles = [r for r in buy_roles if count < r.get("min_orders", 0)]
        if next_roles:
            nxt = next_roles[0]
            ro2 = interaction.guild.get_role(nxt["role_id"])
            need = nxt["min_orders"] - count
            embed.add_field(
                name="⬆️ Role tiếp theo",
                value=f"{ro2.mention if ro2 else nxt['label']} (cần thêm **{need}** đơn)",
                inline=False
            )
...
class SetUserOrdersModal(Modal, title="✏️ Sửa Số Đơn Của User"):
    user_id_input = TextInput(
        label="ID của User",
        placeholder="Chuột phải vào user → Copy ID",
        min_length=15, max_length=20
    ...
    count_input = TextInput(
        label="Số đơn mới",
        placeholder="Nhập số nguyên, ví dụ: 5",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            uid      = int(self.user_id_input.value.strip())
            new_count = int(self.count_input.value.strip())
            if new_count < 0:
                raise ValueError
        except ValueError:
        ...
        if "user_order_count" not in data:
            data["user_order_count"] = {}
        old_count = data["user_order_count"].get(str(uid), 0)
        data["user_order_count"][str(uid)] = new_count
        save_data(data)

        member = interaction.guild.get_member(uid)

        # Give lại role phù hợp
        if member:
            await auto_give_buy_roles(interaction.guild, member, new_count)
        ...
        embed.add_field(name="👤 User",     value=member.mention if member else f"ID:{uid}", inline=True)
        embed.add_field(name="📦 Số đơn cũ", value=f"**{old_count}**",                       inline=True)
        embed.add_field(name="📦 Số đơn mới", value=f"**{new_count}**",                      inline=True)
        embed.set_footer(text=f"Sửa bởi {interaction.user}")
        await interaction.response.send_message(embed=embed, ephemeral=True)


class SyncExistingRolesModal(Modal, title="🔄 Sync Role Cũ → Số Đơn"):
    default_count_input = TextInput(
        label="Số đơn mặc định theo từng bậc role",
        placeholder="VD: 1,5,10 — theo thứ tự role từ thấp→cao (bỏ trống = dùng min_orders)",
        required=False,
        max_length=50
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not buy_roles:
            return await interaction.followup.send(
                "❌ Chưa cấu hình role nào. Hãy thêm role trước.", ephemeral=True
            )

        overwrite = self.overwrite_input.value.strip().lower() in ("yes", "y", "có", "co")

        # Parse số đơn mặc định theo thứ tự role (đã sort tăng dần)
        raw = self.default_count_input.value.strip()
        if raw:
            try:
                default_counts = [int(x.strip()) for x in raw.split(",")]
            except ValueError:
                return await interaction.followup.send(
                    "❌ Định dạng không hợp lệ! VD: `1,5,10`", ephemeral=True
                )
        else:
            default_counts = [r["min_orders"] for r in buy_roles]

        # Đảm bảo đủ số cho từng role
        while len(default_counts) < len(buy_roles):
            default_counts.append(default_counts[-1] if default_counts else 1)
        ...
        for member in guild.members:
            ...
            already = uid_str in data["user_order_count"]

            # Tìm role buy cao nhất member đang có
            highest_idx = -1
            ...
            if highest_idx == -1:
                continue  # Không có role buy nào

            if already and not overwrite:
                skipped += 1
                continue

            target = default_counts[highest_idx]
            old    = data["user_order_count"].get(uid_str, 0)
            data["user_order_count"][uid_str] = target
            synced += 1
            label  = buy_roles[highest_idx].get("label", "?")
            lines.append(f"› {member.display_name} — **{label}** → **{target}** đơn (cũ: {old})")

        save_data(data)

        embed = discord.Embed(
            title="🔄 Sync Role Hoàn Tất",
            color=0x57F287,
            timestamp=datetime.now(timezone.utc)
        )
