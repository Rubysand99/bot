...
async def orderbase_cmd(ctx):
    ...
    embed = discord.Embed(
        title="🏯  Nhận Làm Base Village",
        description=(
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "💰 **Giá chỉ từ `20m` – `35m`** tuỳ theo base bạn chọn\n"
            "━━━━━━━━━━━━━━━━━━━━━━"
        ),
        color=0xE67E22,
        timestamp=datetime.now(timezone.utc)
    ...
    embed.add_field(
        name="<:emoji_17:1483684359415267449>  Cam Kết Khi Mua Base",
        value=(
            "<:emoji_17:1483684359415267449> Tự tìm chỗ xây base luôn\n"
            "<:emoji_17:1483684359415267449> Base có chỗ nhân giống Village\n"
            "<:emoji_17:1483684359415267449> Ít nhất **3 dân làng** bán tu sửa\n"
            "<:emoji_17:1483684359415267449> Có thể có dòng xịn: **Protection, Blast Protection, Fortune...**\n"
            "<:emoji_17:1483684359415267449> Quay video xoá home base khi bàn giao"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="🛡️  Bảo Hành",
        value=(
            "> ⏰ Bảo hành **8 tiếng** kể từ khi mua\n"
            "> 💸 Bị raid trong giờ bảo hành → **hoàn tiền**"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            "```\n"
            "  Gói 20m   │  3 dân làng roll sẵn Tu Sửa\n"
            "            │  Còn lại tự roll\n"
            "────────────┼──────────────────────────────\n"
            "  Gói 35m   │  Roll hết tất cả dân làng\n"
            "            │  Base đẹp & rộng hơn\n"
            "```"
        ),
        inline=False
    ...
    embed.add_field(
        name="📋  Cách Đặt Hàng",
        value=f"Tạo ticket tại <#1464415587378659564> và chọn **🏯 Order Base**",
        inline=False
    )

    embed.set_footer(
        text="TuyTam Store  •  DoанBaoNgoc Seller  •  Yên tâm mà thuê!",
        icon_url=ctx.guild.icon.url if ctx.guild.icon else None
    )
    if ctx.guild.icon:
        embed.set_thumbnail(url=ctx.guild.icon.url)

    await ctx.send("<@&1464411190808805540>", embed=embed)
