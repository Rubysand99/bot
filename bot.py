...
PRICE_TABLE = {
    "sell": {
        ...
        "orderbase":  {"label": "🏯 Order Base",      "unit": "1 base", "price": "Liên hệ",  "note": "Báo yêu cầu cụ thể"},
        "modfixlag":  {"label": "⚡ Mod Fix Lag",     "unit": "1 gói",  "price": "Miễn phí", "note": "Hỗ trợ cài đặt"},
        "giveaway":   {"label": "🎁 Nhận Giveaway",  "unit": "1 lần",  "price": "Miễn phí", "note": "Xác nhận điều kiện"},
        "testtier":   {"label": "🏆 Test Tier",       "unit": "1 lần",  "price": "Liên hệ",  "note": "Liên hệ staff"},
        "support":    {"label": "🆘 Hỗ Trợ",         "unit": "1 lần",  "price": "Miễn phí", "note": "Mọi vấn đề"},
    },
    "buy": {
...
def get_ticket_number():
    # Đọc từ file riêng để counter không bao giờ bị reset
    try:
        with open(TICKET_COUNTER_FILE, "r") as f:
            counter_data = json.load(f)
    except:
        # Nếu file chưa có, lấy giá trị cũ từ data.json để không bị reset về 0
        old = load_data().get("ticket", 0)
        counter_data = {"ticket": old}

    counter_data["ticket"] += 1
    with open(TICKET_COUNTER_FILE, "w") as f:
        json.dump(counter_data, f)

    return f"{counter_data['ticket']:03d}"
...
class OrderModal(Modal):
    def __init__(self, trade_type: str, item_key: str):
        ...
        # Modal đặc biệt cho Order Base
        if item_key == "orderbase":
            self.amount = TextInput(
                label="Loại base / yêu cầu",
                placeholder="Ví dụ: base raid proof, base storage, base PvP...",
                style=discord.TextStyle.paragraph,
                min_length=5, max_length=300
            )
            self.add_item(self.amount)
            self.payment = TextInput(
                label="Ngân sách / Thanh toán",
                placeholder="Ví dụ: 50k MB Bank, 100m ingame...",
                min_length=1, max_length=100
            )
            self.add_item(self.payment)
            self.note = TextInput(
                label="Ghi chú thêm (tuỳ chọn)",
                placeholder="Server nào, deadline, yêu cầu đặc biệt...",
                style=discord.TextStyle.paragraph,
                required=False, max_length=300
            )
            self.add_item(self.note)

        # Modal đặc biệt cho Mod Fix Lag
        elif item_key == "modfixlag":
            self.amount = TextInput(
                label="Phiên bản Minecraft",
                placeholder="Ví dụ: 1.20.1 Fabric / 1.19.4 Forge",
                min_length=3, max_length=50
            )
            self.add_item(self.amount)
            self.payment = TextInput(
                label="Cấu hình máy tính",
                placeholder="Ví dụ: RAM 8GB, CPU i5-10400, GPU GTX 1060",
                min_length=5, max_length=200
            )
            self.add_item(self.payment)
            self.note = TextInput(
                label="Vấn đề đang gặp phải",
                placeholder="Ví dụ: FPS thấp, giật lag khi nhiều entity, chunk load chậm...",
                style=discord.TextStyle.paragraph,
                required=False, max_length=300
            )
            self.add_item(self.note)

        # Modal thông thường cho các item khác
        else:
            self.amount = TextInput(
                label="Số lượng",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Màu & label theo loại ticket
        if self.item_key == "orderbase":
            color = 0xE67E22
            type_label = "🏯 ORDER BASE"
        elif self.item_key == "modfixlag":
            color = 0x1ABC9C
            type_label = "⚡ MOD FIX LAG"
        elif self.item_key == "giveaway":
            color = 0xF1C40F
            type_label = "🎁 NHẬN GIVEAWAY"
        elif self.item_key == "testtier":
            color = 0x9B59B6
            type_label = "🏆 TEST TIER"
        elif self.item_key == "support":
            color = 0x3498DB
            type_label = "🆘 HỖ TRỢ"
        ...
        # Thêm Support Role vào overwrites
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"Staff sẽ xử lý yêu cầu của bạn sớm nhất có thể.\n"
        ...
        embed.add_field(name="📦  Dịch vụ",       value=item_info["label"],            inline=True)

        # Trường tuỳ theo loại ticket
        if self.item_key == "orderbase":
            embed.add_field(name="🏯  Yêu cầu Base",   value=self.amount.value,        inline=False)
            embed.add_field(name="💰  Ngân sách",       value=self.payment.value,       inline=True)
        elif self.item_key == "modfixlag":
            embed.add_field(name="🎮  Phiên bản MC",    value=self.amount.value,        inline=True)
            embed.add_field(name="💻  Cấu hình máy",    value=self.payment.value,       inline=False)
            mod_list = "\n".join(f"• {m}" for m in MOD_FIXLAG_LIST)
            embed.add_field(
                name="📦  Gói mod đề xuất",
                value=f"```{mod_list}```",
                inline=False
            )
        else:
            embed.add_field(name="🔢  Số lượng",        value=self.amount.value,        inline=True)
            # Ước tính giá tự động
        ...
        if hasattr(self, 'note') and self.note.value:
            label = "🔧  Vấn đề" if self.item_key == "modfixlag" else "📝  Ghi chú"
            embed.add_field(name=label, value=self.note.value, inline=False)
        ...
        # Gửi hướng dẫn mod nếu là modfixlag
        if self.item_key == "modfixlag":
            guide_embed = discord.Embed(
                title="📥  Hướng Dẫn Cài Mod",
                description=(
                    "**Bước 1:** Tải & cài [Fabric Installer](https://fabricmc.net/use/installer/)\n"
                    "**Bước 2:** Tải [Fabric API](https://modrinth.com/mod/fabric-api)\n"
                    "**Bước 3:** Bỏ các mod vào thư mục `.minecraft/mods/`\n"
                    "**Bước 4:** Chạy Minecraft với profile Fabric\n\n"
                    "📌 Staff sẽ gửi link tải từng mod phù hợp với phiên bản của bạn!"
                ),
                color=0x1ABC9C
            )
