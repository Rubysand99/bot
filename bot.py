...
class SetupFontSampleModal(Modal, title="✏️ Đổi Tên Kênh Hàng Loạt"):
    sample = TextInput(
        label="Chuỗi mẫu font (copy từ Discord)",
        placeholder="Paste 1-2 chữ mẫu vd: 𝗟𝗲𝗴𝗶𝘁 hoặc 𝐌𝐨𝐧𝐞𝐲",
        max_length=50,
        required=False
    )
    font_name = TextInput(
        label="Hoặc chọn font: normal/bold/sans_bold/script/double",
        placeholder="sans_bold",
        default="sans_bold",
        max_length=20,
        required=False
    )
...
def _detect_font_from_sample(sample: str) -> str:
    """Nhận diện font từ ký tự Unicode mẫu người dùng paste vào."""
    if not sample:
        return "normal"
    c = sample[0]
    cp = ord(c)
