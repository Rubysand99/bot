...
# ================= TIKTOK MONITOR =================
# Dùng RSSHub — ổn định hơn ProxiTok, nhiều instance public miễn phí
# RSS URL: https://rsshub.app/tiktok/user/@{username}
# Tài liệu: https://docs.rsshub.app/routes/social-media#tiktok

import re as _re_tiktok
import aiohttp as _aiohttp_tiktok
import xml.etree.ElementTree as _ET

TIKTOK_CHECK_INTERVAL = 60  # giây — kiểm tra mỗi 60s

# Danh sách RSSHub instance public (fallback lần lượt nếu instance đầu lỗi)
# Có thể thêm instance tự host vào đầu danh sách để ưu tiên
RSSHUB_INSTANCES = [
    "https://rsshub.app",           # instance chính thức — ổn định nhất
    "https://hub.slarker.me",       # mirror public
    "https://rsshub.rssforever.com",# mirror public
    "https://rsshub.feeded.xyz",    # mirror public
]
...
async def _fetch_tiktok_rss(username: str) -> str | None:
    """
    Thử lấy RSS TikTok từ nhiều RSSHub instance.
    RSSHub route: /tiktok/user/@{username}
    """
        ...
        try:
            async with _aiohttp_tiktok.ClientSession() as session:
                async with session.get(
                    ...
                    timeout=_aiohttp_tiktok.ClientTimeout(total=20),
                    allow_redirects=True
                ) as resp:
                    if resp.status == 200:
                        text = await resp.text()
                        # Kiểm tra có phải RSS hợp lệ không
                        if "<rss" in text or "<feed" in text or "<channel" in text:
                            print(f"[TikTok] ✅ RSS lấy thành công @{username} từ {base}")
                            return text
        except Exception as e:
            print(f"[TikTok] ⚠️ {base} lỗi: {e}")
            continue
    print(f"[TikTok] ❌ Không lấy được RSS @{username} từ tất cả instance")
    return None

def _parse_rss_latest_video(xml_text: str):
    """
    Trả về (video_id, title, url, thumbnail) của video mới nhất.
    Hỗ trợ cả RSS 2.0 và Atom feed từ RSSHub.
    """
    try:
        ...
        if channel is not None:
            ...
            # Thumbnail: media:thumbnail → media:content → enclosure
        ...
        # ── Atom feed ──
            ...
            if entry is None:
                return None
            title = entry.findtext(f"{{{atom_ns}}}title") or "Video mới"
            link_el = entry.find(f"{{{atom_ns}}}link")
            link = link_el.get("href", "") if link_el is not None else ""
            # Thumbnail từ media:thumbnail
            el = entry.find("media:thumbnail", ns)
            if el is not None:
                thumb = el.get("url")
        ...
        # Rút video_id từ URL dạng .../video/123456789
        m = _re_tiktok.search(r"/video/(\d+)", link)
        video_id = m.group(1) if m else link

        return video_id, title, link, thumb

    except Exception as e:
        print(f"[TikTok] Lỗi parse RSS: {e}")
        return None

async def _check_tiktok_live(username: str) -> bool:
    """
    Kiểm tra LIVE qua trang HTML TikTok.
    Trả về True nếu đang LIVE.
    Dùng nhiều dấu hiệu để tránh false positive.
    """
    ...
    try:
        async with _aiohttp_tiktok.ClientSession() as session:
            async with session.get(
                ...
                # Nếu bị redirect về trang profile → không live
                final_url = str(resp.url).rstrip("/")
                if final_url.endswith(f"@{username}") or final_url.endswith(f"@{username}/"):
                    return False
                if resp.status != 200:
                    return False
                # Chỉ đọc 5KB đầu để tiết kiệm bandwidth
                chunk = await resp.content.read(5120)
                text  = chunk.decode("utf-8", errors="ignore")
                # Các dấu hiệu LIVE thực sự
                live_signals = (
                    '"liveRoomInfo"' in text,
                    '"statusCode":0'  in text,
                    '"roomStatus":2'  in text,   # 2 = đang live
                    '"isLiving":true' in text,
                )
                return any(live_signals)
    except Exception as e:
        print(f"[TikTok] Lỗi check live @{username}: {e}")
    return False

async def _tiktok_monitor_loop():
    """Background task kiểm tra video mới & LIVE mỗi TIKTOK_CHECK_INTERVAL giây."""
    await bot.wait_until_ready()
    print("[TikTok] ✅ Monitor loop đã khởi động")
    ...
    while not bot.is_closed():
        try:
            ...
            if not accounts or not notify_ch_id:
                await asyncio.sleep(TIKTOK_CHECK_INTERVAL)
                continue
            ...
            for username in accounts:
                ...
                if rss:
                    ...
                    if result:
                        ...
                        if prev_id is None:
                            # Lần đầu gặp → chỉ lưu, không thông báo
                            ...
                            changed = True
                            embed = discord.Embed(
                                title=f"📹 @{username} vừa đăng video mới!",
                                description=f"**{title}**",
                                url=link,
                                color=0xFE2C55,
                                timestamp=datetime.now(timezone.utc)
                            ...
                            embed.set_footer(text="TikTok Notification")
                            try:
                                await notify_ch.send(embed=embed)
                                print(f"[TikTok] 📹 Video mới @{username}: {title}")
                            except Exception as e:
                                print(f"[TikTok] ❌ Gửi thông báo video lỗi: {e}")

                # ── Kiểm tra LIVE ──
                is_live_now  = await _check_tiktok_live(username)
                ...
                if is_live_now and not was_live:
                    ...
                    changed = True
                    embed = discord.Embed(
                        title=f"🔴 @{username} đang LIVE trên TikTok!",
                        description=f"👉 [Xem ngay](https://www.tiktok.com/@{username}/live)",
                        color=0xFF0000,
                        timestamp=datetime.now(timezone.utc)
                    )
                    ...
                    try:
                        await notify_ch.send(embed=embed)
                        print(f"[TikTok] 🔴 LIVE bắt đầu @{username}")
                    except Exception as e:
                        print(f"[TikTok] ❌ Gửi thông báo LIVE lỗi: {e}")
                elif not is_live_now and was_live:
                    # Kết thúc LIVE — reset trạng thái (không thông báo theo yêu cầu)
                    live_status[username] = False
                    changed = True
                    print(f"[TikTok] ⚫ LIVE kết thúc @{username}")

                await asyncio.sleep(3)  # delay nhỏ giữa từng tài khoản

            if changed:
                data["tiktok_last_video"]  = last_video
                data["tiktok_live_status"] = live_status
                save_data(data)

        except Exception as e:
            print(f"[TikTok] ❌ Lỗi monitor loop: {e}")
...
async def tiktok_cmd(ctx, sub: str = None, *, arg: str = None):
    ...
    if sub == "add":
        ...
        save_data(data)
        await ctx.reply(f"✅ Đã xoá `@{username}` khỏi danh sách theo dõi.")
        ...
        embed.add_field(name="⏱️ Interval",        value=f"{TIKTOK_CHECK_INTERVAL}s", inline=True)
        embed.add_field(
            name="📌 Tính năng bật",
            value="📹 Video mới  |  🔴 LIVE alert",
            inline=False
        )
        if accounts:
            live_status = data.get("tiktok_live_status", {})
            lines = [f"• `@{u}` {'🔴 LIVE' if live_status.get(u) else '⚫'}" for u in accounts]
...
async def on_ready():
    ...
    bot.loop.create_task(_tiktok_monitor_loop())
    print("[TikTok] 🚀 Monitor task đã được khởi tạo")
