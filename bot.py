...
async def auto_ping_unclaimed(channel, number, guild):
    ...
    if not claimed:
        try:
            await ch.send(
                f"⏰ <@&{get_cfg_support_role()}> Ticket **#{number}** chưa có staff nhận sau 5 phút! Vui lòng xử lý sớm."
            )
        except:
            pass
...
async def help_cmd(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`.close` — Đóng ticket *(admin/staff)*\n"
            "`.addnote <nội dung>` — Ghi chú nội bộ *(admin/staff)*"
        ),
        inline=False
    )
    embed.add_field(
        name="🛡️  Kiểm duyệt",
        value=(
            "`.clear <số>` — Xoá tin nhắn *(admin)*\n"
            "`.addrole @user @role` — Thêm role *(admin)*\n"
            "`.removerole @user @role` — Xoá role *(admin)*\n"
            "`.createchannel <tên>` — Tạo kênh text *(admin)*\n"
    ...
    embed.add_field(
        name="🎉  Giveaway",
        value=(
            "`.giveaway <time> <số người> <phần thưởng>` *(admin)*\n"
            "`.gend <message_id>` — Kết thúc sớm *(admin)*\n"
            "`.greroll <message_id>` — Quay lại người thắng *(admin)*\n"
            "Ví dụ: `.giveaway 1h 2 100m ingame`"
        ),
        inline=False
    )
    embed.add_field(
        ...
        value=(
            ...
            "`.stock <item> <số lượng>` — Cập nhật *(admin/staff)*"
        ),
        inline=False
    )
    embed.add_field(
