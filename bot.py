...
async def _run_live_watcher(username: str):
    ...
    while True:
        ...
        async def on_disconnect(event: DisconnectEvent):
            ...
            embed = discord.Embed(
                title=f"⚫ @{username} đã kết thúc LIVE",
                description=f"Cảm ơn mọi người đã xem! Theo dõi `@{username}` để không bỏ lỡ lần sau.",
                color=0x555555,
                timestamp=datetime.now(timezone.utc)
            )
        ...
        try:
            ...
            err_msg = str(e).lower()
            if not any(k in err_msg for k in ("not live", "offline", "could not find", "failed to retrieve")):
