...
# ================= TIKTOK MONITOR =================
# Dùng RSS của Proxitok (không cần API key TikTok)
# RSS URL: https://proxitok.pabloferreiro.es/@{username}/rss
# Fallback: https://rss.app hoặc feedburner proxy
...
async def _fetch_tiktok_rss(username: str) -> str | None:
    """Thử lấy RSS TikTok từ nhiều ProxiTok instance."""
...
def _parse_rss_latest_video(xml_text: str):
    ...
    try:
        ...
        # Lấy thumbnail từ media:thumbnail hoặc media:content
        thumb = None
...
async def _check_tiktok_live(username: str) -> bool:
    """
    Kiểm tra LIVE qua page HTML của TikTok (không cần API).
    ...
    try:
        async with _aiohttp_tiktok.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=_aiohttp_tiktok.ClientTimeout(total=15), allow_redirects=True) as resp:
                text = await resp.text()
                # Nếu trang LIVE tồn tại và có chứa dấu hiệu stream
                if resp.status == 200 and ('"statusCode":0' in text or '"liveRoomInfo"' in text or 'LIVE' in text[:3000]):
                    # Nếu bị redirect về trang profile → không live
                    if str(resp.url).rstrip("/").endswith(f"@{username}"):
                        return False
                    return True
    except Exception:
        pass
