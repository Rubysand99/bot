...
def get_sellers() -> list:
    """Trả về danh sách seller từ data hoặc SELLERS mặc định."""
    data = load_data()
    saved = data.get("sellers")
...
# ── SELLERS: danh sách seller hiển thị trong nút mua/bán ──
# Mỗi seller: {"id": discord_user_id, "label": "Tên hiển thị", "emoji": "emoji tuỳ chọn"}
# Buyer chọn seller → tên ticket = (label seller)-(số ticket)
SELLERS: list[dict] = [
    # Ví dụ: {"id": 846332174734983219, "label": "TuyTam", "emoji": "🏪"},
    # Thêm seller vào đây hoặc cấu hình động qua .settings -> Seller
]
...
async def create_order_ticket(interaction: discord.Interaction, trade_type: str,
    ...
    try:
        ...
        # Thêm quyền cho seller được chọn
        if seller_id:
...
class SellerSelect(Select):
    def __init__(self, trade_type: str):
        ...
        if sellers:
            ...
            # Fallback nếu chưa cấu hình seller nào
            options = [
                discord.SelectOption(label="Staff (mặc định)", value="0", emoji="🙋")
            ]
        action = "mua" if trade_type == "sell" else "bán"
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            await create_order_ticket(
                ...
                seller_id=int(seller_id_str) if seller_id_str != "0" else None,
...
_DEFAULT_PRICE_SECTIONS = [
    {
        ...
        "content": (
            ...
            "> - **acc online hỏi <@427399742906040333>**"
        ),
    },
    {
        "key": "robux",
