...
async def done_cmd(ctx, amount_str: str = None):
    ...
    embed.set_footer(text=f"Xác nhận bởi {ctx.author.display_name}")
    await ctx.reply(embed=embed)
...
async def _call_groq(user_id: int, user_message: str) -> str:
    """Gọi Groq API với lịch sử hội thoại của user (tối đa 10 lượt)."""
    if not GROQ_API_KEY:
        return "❌ Chưa cài `GROQ_API_KEY` trong biến môi trường."

    # Cập nhật lịch sử — Groq dùng OpenAI-compatible format
    history = _ai_chat_history.setdefault(user_id, [])
    history.append({"role": "user", "content": user_message})

    # Giữ tối đa AI_HISTORY_LIMIT lượt (mỗi lượt = 2 phần tử)
    ...
    try:
        async with _aiohttp_ai.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    err = await resp.text()
                    print(f"[AI] ❌ Groq lỗi {resp.status}: {err[:200]}")
                    return "⚠️ AI tạm thời không phản hồi. Vui lòng thử lại sau."
        ...
        return "⚠️ Lỗi kết nối tới AI. Vui lòng thử lại sau."
...
async def ai_cmd(ctx, *, prompt: str = None):
    ...
    if sub == "tomtat":
        ...
        async with ctx.typing():
            ...
            chat_log = "\n".join(f"{m.author.display_name}: {m.content}" for m in msgs if m.content)[:4000]
            task = (
    ...
    if sub == "phantich":
        ...
        async with ctx.typing():
            ...
            if len(msgs) < 5:
                return await ctx.reply(f"❌ Không đủ tin nhắn của {target.display_name} để phân tích (cần ít nhất 5).")
            sample = "\n".join(m.content for m in msgs[:50])[:3000]
            task = (
                f"Phân tích phong cách chat của người dùng Discord tên '{target.display_name}' "
            ...
            reply = await _call_groq(ctx.author.id, task)
        embed = discord.Embed(
            title=f"🔍 Phân tích phong cách chat của {target.display_name}",
...
async def invite_cmd(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(
        title=f"📨 Invite của {target.display_name}",
        color=0x5865F2,
        timestamp=datetime.now(timezone.utc)
    )
    embed.set_thumbnail(url=target.display_avatar.url)
...
async def resetinvite_cmd(ctx, member: discord.Member = None):
    ...
    if is_all:
        ...
        await ctx.reply(f"✅ Đã reset invite của **{member.display_name}**.")
    else:
        await ctx.reply(
            "❌ Dùng:\n"
            "`.resetinvite @user` — reset 1 người\n"
...
class SyncExistingRolesModal(Modal, title="🔄 Sync Role Cũ → Tổng Tiền"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for member in guild.members:
            ...
            lines.append(f"› {member.display_name} — **{label}** → **{fmt_amount(target)}** (cũ: {fmt_amount(old)})")
