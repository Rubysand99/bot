...
async def emoji_cmd(ctx, *, args: str = None):
    ...
    if args:
        ...
        pattern = r"<(a?):([^:>]+):(\d+)>"
        matches = _re_emoji.findall(args, )
        if not matches:
        ...
        for animated, name, emoji_id in matches:
            ...
            try:
                async with ctx.bot.http._HTTPClient__session.get(url) as resp:
                    if resp.status != 200:
        ...
        lines = []
        if added:
            lines.append(f"✅ Đã thêm {len(added)} emoji: {' '.join(added)}")
        if failed:
            lines.append(f"❌ Thất bại {len(failed)}: {', '.join(failed)}")
        return await ctx.reply("\n".join(lines) if lines else "Không có gì được thêm.")
