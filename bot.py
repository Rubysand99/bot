...
# ================= LỆNH .SV — Dịch Vụ Setup Server Discord =================
@bot.command(name="sv", aliases=["dichvu", "service"])
async def sv_command(ctx: commands.Context):
    """Gửi embed giới thiệu dịch vụ Setup Server Discord."""
    embed = discord.Embed(
        title="🛠️  Dịch Vụ Setup Server Discord",
        description=(
            "Chuyên nghiệp · Thẩm mỹ · Tối ưu · Dễ mở rộng\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ),
        color=0x5865F2,
        timestamp=datetime.now(timezone.utc),
    ...
    embed.add_field(
        name="📦  Phù Hợp Với",
        value=(
            "• Cộng đồng game, học tập, nhóm bạn bè\n"
            "• Shop buôn bán, tiền ảo, trading\n"
            "• Mọi nhu cầu theo yêu cầu riêng"
        ),
        inline=False,
    )

    embed.add_field(
        name="✅  Chất Lượng Cam Kết",
        value=(
            "• Thiết kế đúng nhu cầu — logic rõ ràng, dễ dùng\n"
            "• Dùng bot mạnh: Dyno, MEE6, Carl-bot, AutoMod…\n"
            "• Giao diện gọn gàng, thành viên mới dễ hoà nhập\n"
            "• Tối ưu quyền & vai trò — chống spam, troll"
        ),
        inline=False,
    )
    ...
    embed.add_field(
        name="💰  Giá",
        value="Thoả thuận thoải mái — deal đến khi hai bên hài lòng 😊",
        inline=False,
    )
    ...
    embed.add_field(
        name="🔧  Quy Trình",
        value=(
            "1️⃣  Bạn mô tả ý tưởng / nhu cầu server\n"
            "2️⃣  Shop tư vấn sơ bộ & báo giá\n"
            "3️⃣  Tiến hành setup — bàn giao tận tay"
        ),
        inline=False,
    )
    ...
    embed.add_field(
        name="📩  Liên Hệ",
        value="Mở ticket hoặc nhắn trực tiếp để được tư vấn miễn phí!",
        inline=False,
    )
