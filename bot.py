...
async def _close_ticket(channel, bot_instance, closer: discord.Member = None):
    ...
    # Gửi transcript vào kênh TRANSCRIPT_CHANNEL_ID
    if transcript_ch:
    ...
    # Gửi log (không kèm file) vào log_channel chung nếu khác kênh transcript
    if log and log != transcript_ch:
        await log.send(embed=embed)
...
async def _call_gemini(user_id: int, user_message: str) -> str:
    """Gọi Gemini API với lịch sử hội thoại của user (tối đa 10 lượt)."""
    if not GEMINI_API_KEY:
        return "❌ Chưa cài `GEMINI_API_KEY` trong biến môi trường."

    # Cập nhật lịch sử — Gemini dùng role "user" / "model"
    ...
    # Gemini v1beta hỗ trợ systemInstruction (camelCase) thay vì system_instruction
    payload = {
    ...
    try:
        async with _aiohttp_ai.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    err = await resp.text()
                    print(f"[AI] ❌ Gemini lỗi {resp.status}: {err[:200]}")
                    return "⚠️ AI tạm thời không phản hồi. Vui lòng thử lại sau."
                data = await resp.json()
                reply = data["candidates"][0]["content"]["parts"][0]["text"]

        # Lưu reply vào lịch sử
        _ai_chat_history[user_id].append({"role": "model", "parts": [{"text": reply}]})
        return reply
...
async def handle_ai_message(message: discord.Message):
    ...
    async with message.channel.typing():
        reply = await _call_gemini(message.author.id, message.content)
