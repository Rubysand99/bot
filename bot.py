...
HELP_CATEGORIES = {
    ...
    "kenh": {
        ...
        "fields": [
            ...
            ("Price Channel",   "ID cài qua `.settings` → bảng giá tự cập nhật"),
...
_SLASH_SUMMARY = (
    "`/ping` `/price` `/qr` `/botinfo` `/serverinfo` `/userinfo`\n"
...
async def help_cmd(ctx, category: str = None):
    if category is None:
        ...
        cats = [
            ...
            ("📌 `.help kenh`",     "Data, Proof, Legit, Balance, Counter, Price, Log channel IDs"),
        ]
...
class SettingsView(View):
    ...
    @discord.ui.button(label="💲 Price Channel", style=discord.ButtonStyle.grey, row=3)
    async def change_price(self, interaction: discord.Interaction, button: Button):
        if interaction.user.id not in ADMIN_IDS:
            return await interaction.response.send_message("❌ Chỉ admin.", ephemeral=True)
        await interaction.response.send_modal(ChangeChannelModal("cfg_price_channel", "Price Channel"))
...
async def settings(ctx):
    ...
    price_id    = get_cfg_price_channel()
    qr_path     = get_qr_path()
    buy_roles   = get_buy_roles()
    ...
    price_val   = f"<#{price_id}>" if price_id else "❌ Chưa cài — nhấn **💲 Price Channel**"
    ...
    embed.add_field(name="💲  Price Channel",    value=price_val,    inline=True)
...
async def handle_vouch_message(message: discord.Message):
    ...
    Đồng thời tìm buyer từ ticket đang mở và give role mua hàng tự động.
    """
    ...
    # ── Auto-give role mua hàng ──
    # Tìm user từ mention hoặc reply trong tin nhắn done
    # Ưu tiên: reply → mention → tìm ticket đang mở gần nhất của member nào đó
    guild = message.guild
    buyer: discord.Member | None = None

    # 1. Nếu tin nhắn done có reply → đọc author của tin nhắn gốc
    if message.reference and message.reference.message_id:
        try:
            ref_msg = await message.channel.fetch_message(message.reference.message_id)
            if ref_msg and not ref_msg.author.bot:
                buyer = guild.get_member(ref_msg.author.id)
        except Exception:
            pass

    # 2. Nếu tin nhắn done có mention user
    if not buyer and message.mentions:
        for m in message.mentions:
            if not m.bot:
                buyer = guild.get_member(m.id)
                break

    # 3. Tìm ticket đang mở gần nhất (kênh có topic chứa |sell| hoặc |buy| và đang open)
    if not buyer:
        cat_id = get_cfg_category()
        category = discord.utils.get(guild.categories, id=cat_id) if cat_id else None
        ticket_channels = [
            ch for ch in guild.text_channels
            if ch.topic and "|" in ch.topic and ch.name.startswith("ticket-")
            and (category is None or ch.category_id == cat_id)
        ]
        if ticket_channels:
            # Lấy ticket mới nhất (số lớn nhất)
            ticket_channels.sort(key=lambda c: c.name, reverse=True)
            for tc in ticket_channels[:5]:
                parts = tc.topic.split("|")
                try:
                    uid = int(parts[0])
    ...
    # Tăng số đơn và give role
    new_order_count = increment_user_order_count(buyer.id)
    highest_role_cfg = await auto_give_buy_roles(guild, buyer, new_order_count)

    # Thông báo trong kênh proof
    if highest_role_cfg:
        role_obj = guild.get_role(highest_role_cfg.get("role_id", 0))
        label = highest_role_cfg.get("label", "buy role")
        try:
            await message.channel.send(
                f"🏆 {buyer.mention} đã hoàn thành **{new_order_count}** đơn "
                f"→ nhận role **{role_obj.mention if role_obj else label}**!",
                delete_after=15
            )
        except Exception:
            pass
    else:
        # Đã +1 đơn nhưng chưa đủ role nào
        buy_roles = get_buy_roles()
        if buy_roles:
            next_role = buy_roles[0]
            need = next_role.get("min_orders", 1) - new_order_count
            try:
                await message.channel.send(
                    f"📦 {buyer.mention} — **{new_order_count}** đơn "
