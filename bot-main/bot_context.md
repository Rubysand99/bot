...
**Deploy:** Railway ← GitHub `main` | **Repo:** github.com/Rubysand99/bot | **DB:** MongoDB Atlas `tuytam_bot.bot_data` (_id:"main") + `tuytam_bot.giveaways`
...
### [v3.8.2] 2026-05-29
- `logger.py` — Thay `bot.get_channel()` → `get_or_fetch_channel()` đúng convention (tránh miss cache)
- `logger.py` — Fix `interaction.channel.mention` → `getattr(..., "mention", "N/A")` tránh lỗi PartialMessageable/DM
- `logger.py` — `.setuplog` nhận arg `[category_id]` tùy chọn thay vì hardcode ID
- `logger.py` — Import thêm `get_or_fetch_channel` từ `core.data`
