...
HELP_CATEGORIES = {
    ...
    "mod": {
        ...
        "fields": [
            ("`.clear <số>`", "Xoá tin nhắn (tối đa 500) *(admin)*"),
    ...
    "game": {
        "title": "🎮  Lệnh Trò Chơi",
        "fields": [
            ("`.cf [số tiền]`", "Tung đồng xu — heads/tails"),
            ("`.roll [xdY]`", "Xúc xắc — ví dụ: `.roll 2d6`"),
            ("`.rps <kéo/búa/bao>`", "Oẳn tù tì với bot"),
            ("`.guess`", "Đoán số bí mật (1-100)"),
            ("`.8ball <câu hỏi>`", "Hỏi quả cầu 8 huyền bí"),
            ("`.slots`", "Máy quay thưởng 🎰"),
        ]
    },
    "caidat": {
        ...
        "fields": [
            ...
            ("`.settings` / `.st`", "Cấu hình & đổi ID kênh, QR *(admin)*"),
...
async def help_cmd(ctx, category: str = None):
    if category is None:
        ...
        cats = [
            ...
            ("🎮 `.help game`",     "Trò chơi: coinflip, xúc xắc, oẳn tù tì..."),
            ("⚙️ `.help caidat`",   "Cài đặt panel, kênh, QR"),
        ]
...
async def slash_gend(interaction: discord.Interaction, message_id: str):
    ...
    await end_giveaway(gw["channel_id"], mid, gw["winners"])
...
async def coinflip_cmd(ctx, bet: str = None):
    result = _random.choice(["Heads 👑", "Tails 🪙"])
    color  = 0xF1C40F if "Heads" in result else 0x5865F2
    embed  = discord.Embed(title="🪙  Tung Đồng Xu", color=color, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Kết quả", value=f"**{result}**", inline=True)
    if bet:
        embed.add_field(name="Bạn cược", value=bet, inline=True)
...
async def roll_cmd(ctx, dice: str = "1d6"):
    """Ví dụ: .roll 2d6  .roll d20  .roll 3d8"""
    try:
        ...
        return await ctx.reply("❌ Dùng: `.roll 2d6` (tối đa 20 xúc xắc, tối đa 1000 mặt)")

    rolls  = [_random.randint(1, sides) for _ in range(count)]
    total  = sum(rolls)
    detail = " + ".join(str(r) for r in rolls) if count > 1 else str(rolls[0])

    embed = discord.Embed(title="🎲  Xúc Xắc", color=0xE67E22, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="🎲 Kết quả", value=f"**{detail}**" + (f" = **{total}**" if count > 1 else ""), inline=False)
    embed.add_field(name="📊 Xúc xắc", value=f"`{count}d{sides}`", inline=True)
    if count > 1:
        embed.add_field(name="🏆 Tổng", value=f"**{total}**", inline=True)
    embed.set_footer(text=f"Roll bởi {ctx.author.display_name}")
    await ctx.reply(embed=embed)


@bot.command(name="rps")
async def rps_cmd(ctx, choice: str = None):
    """Oẳn tù tì: .rps kéo | .rps búa | .rps bao"""
    if not choice:
        return await ctx.reply("❌ Dùng: `.rps kéo` / `.rps búa` / `.rps bao`")

    mapping = {"keo": "kéo", "kéo": "kéo", "bua": "búa", "búa": "búa", "bao": "bao"}
    player = mapping.get(choice.lower())
    if not player:
        return await ctx.reply("❌ Chọn: `kéo`, `búa`, hoặc `bao`")

    icons  = {"kéo": "✌️", "búa": "✊", "bao": "🖐️"}
    bot_choice = _random.choice(["kéo", "búa", "bao"])

    wins = {"kéo": "bao", "búa": "kéo", "bao": "búa"}
    if player == bot_choice:
        result, color = "🤝 Hoà!", 0xFEE75C
    elif wins[player] == bot_choice:
        result, color = "🎉 Bạn thắng!", 0x57F287
    else:
        result, color = "😈 Bot thắng!", 0xED4245

    embed = discord.Embed(title="✊✌️🖐️  Oẳn Tù Tì", description=result, color=color, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Bạn",  value=f"{icons[player]} {player.capitalize()}",     inline=True)
    embed.add_field(name="Bot",  value=f"{icons[bot_choice]} {bot_choice.capitalize()}", inline=True)
    embed.set_footer(text=f"Chơi bởi {ctx.author.display_name}")
    await ctx.reply(embed=embed)


# Lưu game đoán số đang chạy: user_id → {"secret": int, "attempts": int}
_guess_games: dict[int, dict] = {}

@bot.command(name="guess")
async def guess_cmd(ctx, number: str = None):
    """Đoán số bí mật 1-100. Dùng .guess để bắt đầu, .guess <số> để đoán."""
    uid = ctx.author.id

    if number is None:
        # Bắt đầu game mới
        _guess_games[uid] = {"secret": _random.randint(1, 100), "attempts": 0}
        embed = discord.Embed(
            title="🔢  Đoán Số Bí Mật",
            description="Tôi đã nghĩ ra một số từ **1 đến 100**!\nDùng `.guess <số>` để đoán. Bạn có **10 lượt**.",
            color=0x9B59B6,
            timestamp=datetime.now(timezone.utc)
        )
        embed.set_footer(text=f"Game của {ctx.author.display_name}")
        return await ctx.reply(embed=embed)

    if uid not in _guess_games:
        return await ctx.reply("❌ Bạn chưa bắt đầu game! Dùng `.guess` để bắt đầu.")

    try:
        guess = int(number)
        if guess < 1 or guess > 100: raise ValueError
    except:
        return await ctx.reply("❌ Nhập số từ 1 đến 100!")

    game = _guess_games[uid]
    game["attempts"] += 1
    secret = game["secret"]
    left   = 10 - game["attempts"]

    if guess == secret:
        del _guess_games[uid]
        embed = discord.Embed(
            title="🎉  Đúng Rồi!",
            description=f"Số bí mật là **{secret}**!\nBạn đoán đúng sau **{game['attempts']} lượt**! 🏆",
            color=0x57F287
        )
    elif left <= 0:
        del _guess_games[uid]
        embed = discord.Embed(
            title="💀  Hết Lượt!",
            description=f"Số bí mật là **{secret}**. Chúc may mắn lần sau!\nDùng `.guess` để chơi lại.",
            color=0xED4245
        )
    else:
        hint = "📈 Cao hơn!" if guess < secret else "📉 Thấp hơn!"
        embed = discord.Embed(
            title="🔢  Đoán Số",
            description=f"**{guess}** — {hint}\nCòn **{left} lượt**.",
            color=0x9B59B6
        )
        embed.set_footer(text=f"Game của {ctx.author.display_name}")
...
async def eightball_cmd(ctx, *, question: str = None):
    if not question:
        return await ctx.reply("❌ Dùng: `.8ball <câu hỏi của bạn>`")

    answers = [
        ("✅ Chắc chắn rồi!", 0x57F287),
        ("✅ Hoàn toàn có thể!", 0x57F287),
        ("✅ Theo quan điểm của tôi, có!", 0x57F287),
        ("✅ Có vẻ vậy!", 0x57F287),
        ("✅ Có thể lắm!", 0x57F287),
        ("🤔 Hãy hỏi lại sau.", 0xFEE75C),
        ("🤔 Bây giờ tôi không thể nói.", 0xFEE75C),
        ("🤔 Tập trung rồi hỏi lại.", 0xFEE75C),
        ("🤔 Khó nói lắm.", 0xFEE75C),
        ("❌ Đừng trông chờ vào đó.", 0xED4245),
        ("❌ Câu trả lời của tôi là không.", 0xED4245),
        ("❌ Nguồn tin của tôi nói không.", 0xED4245),
        ("❌ Triển vọng không tốt lắm.", 0xED4245),
    ]
    answer, color = _random.choice(answers)
    embed = discord.Embed(title="🎱  Quả Cầu 8 Huyền Bí", color=color, timestamp=datetime.now(timezone.utc))
...
async def slots_cmd(ctx):
    ...
    if s1 == s2 == s3 == "7️⃣":
        outcome, color = "🏆 **JACKPOT!!!** Ba số 7 — Chiến thắng vĩ đại!", 0xF1C40F
    elif s1 == s2 == s3 == "💎":
        outcome, color = "💎 **BA KIM CƯƠNG!** Thắng lớn!", 0x00BFFF
    elif s1 == s2 == s3:
        outcome, color = f"🎉 **BA {s1}!** Thắng!", 0x57F287
    elif s1 == s2 or s2 == s3 or s1 == s3:
        outcome, color = "✨ Hai ký hiệu trùng — Nhỏ thắng!", 0xFEE75C
    else:
        outcome, color = "❌ Không trùng — Thua rồi, thử lại!", 0xED4245

    embed = discord.Embed(title="🎰  Máy Quay Thưởng", description=result_str, color=color, timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Kết quả", value=outcome, inline=False)
    embed.set_footer(text=f"Quay bởi {ctx.author.display_name}")
