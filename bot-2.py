...
# Cooldown: lưu thời điểm gửi request gần nhất của từng user
ai_last_used: dict[int, float] = {}
AI_COOLDOWN_SECONDS = 5  # Giới hạn mỗi user 1 tin mỗi 5 giây
        ...
        "system_instruction": {
            "parts": [{
                "text": (
                    ...
                    "Nếu được hỏi về giá hoặc giao dịch, hướng dẫn người dùng tạo ticket. "
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=20)) as resp:
                if resp.status == 429:
                    # Xoá tin nhắn vừa thêm vào history vì chưa được xử lý
                    ai_history[user_id].pop()
                    return (
                        "⏳ **Bot đang bận!** Gemini free tier giới hạn số request mỗi phút.\n"
                        "Vui lòng chờ **15-30 giây** rồi thử lại nhé!"
                    )
                if resp.status != 200:
                    ai_history[user_id].pop()
                    return f"❌ Có lỗi xảy ra (`{resp.status}`), vui lòng thử lại sau!"
        ...
        if ai_history[user_id]:
            ai_history[user_id].pop()
        return "❌ Không kết nối được Gemini, thử lại sau!"
...
async def on_message(message: discord.Message):
    ...
    import time
    now = time.time()
    last = ai_last_used.get(message.author.id, 0)
    remaining = AI_COOLDOWN_SECONDS - (now - last)
    if remaining > 0:
        await message.reply(
            f"⏳ Chờ **{remaining:.1f}s** trước khi gửi tiếp nhé!",
            delete_after=3
        )
        return
    ai_last_used[message.author.id] = now
