...
froasync def _tiktok_monitor_loop():m dotenv import load_dotenv
...
def parse_latest_video(rss_text):
    try:
...
TIKTOK_CHECK_INTERVAL = 60

RSSHUB_INSTANCES = [
    "https://rsshub.app",
    "https://rsshub.rssforever.com",
    "https://rsshub.moeyy.xyz",   # thêm cái này
    # ❌ xoá feeded.xyz
]
...
async def get_session():
    ...
    if _session is None or _session.closed:
        _session = _aiohttp_tiktok.ClientSession()
    return _session
    ...
    for base in RSSHUB_INSTANCES:
        ...
        try:
            async with session.get(
                ...
                timeout=_aiohttp_tiktok.ClientTimeout(total=10)
            ) as resp:
                ...
                if resp.status == 200:
                    ...
                    if "<rss" in text or "<feed" in text:
                        print(f"[TikTok] ✅ @{username} OK từ {base}")
                        return text

                else:
                    print(f"[TikTok] ⚠️ {base} status {resp.status}")

        except Exception as e:
            print(f"[TikTok] ⚠️ {base} lỗi: {e}")

    print(f"[TikTok] ❌ @{username} FAIL toàn bộ instance")
...
async def _tiktok_monitor_loop():
    ...
    while not bot.is_closed():
        ...
        channel_id = data.get("tiktok_notify_channel")
        accounts = data.get("tiktok_accounts", [])
        last_videos = data.get("tiktok_last_video", {})
        ...
        for username in accounts:
            ...
            # 🔥 chống gửi trùng
            if last_videos.get(username) == link:
                continue

            # lưu lại
            last_videos[username] = link
            data["tiktok_last_video"] = last_videos
            await save_data_async(data)

            # gửi Discord
            await channel.send(f"📱 @{username} đăng video mới!\n🎥 {title}\n🔗 {link}")
