...
async def get_next_ticket_number() -> str:
    """Lấy số ticket tiếp theo — lưu vào data Discord."""
    data = load_data()
    next_num = data.get("ticket", 0) + 1
    data["ticket"] = next_num
    save_data(data)
    return f"{next_num:03d}"

async def sync_ticket_counter(guild: discord.Guild):
    """Khi bot khởi động: lấy số cao nhất giữa data Discord và tên channel ticket-XXX."""
    data = load_data()
    max_num = data.get("ticket", 0)

    # Quét tên channel thực tế để tránh trùng sau khi restore
...
async def _close_ticket(channel, bot_instance, closer: discord.Member = None):
    ...
    if log:
        ...
        embed.add_field(name="👤 Người tạo",      value=creator.mention if creator else f"`ID:{user_id}`",     inline=True)
